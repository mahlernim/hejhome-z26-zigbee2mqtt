...
Comparison with other Zigbee devices found that parts of the network were already unavailable before these episodes. However, an independent Zigbee power-reporting device continued producing changing readings throughout all three windows with gaps no longer than 45 seconds. Other sensors also updated, and another bulb changed state during the September 3 windows. This rules out a sustained whole-network outage during those periods, while leaving partial mesh problems possible.
...
The [official contribution guide](https://www.zigbee2mqtt.io/advanced/support-new-devices/01_support_new_devices.html) describes submitting the definition to the converter repository and a device image plus optional notes to `zigbee2mqtt.io`. On September 5, this project submitted [converter PR #13122](https://github.com/Koenkk/zigbee-herdsman-converters/pull/13122) and [device image and documentation PR #5490](https://github.com/Koenkk/zigbee2mqtt.io/pull/5490). They explain the keep-alive requirement, the empirically tested 120-second interval, GOQUAL's 30-second choice, and the unresolved reliability limits. Local validation passed the converter build, lint, all 844 existing tests, benchmarks, and separate fingerprint and polling-lifecycle checks. The submissions await maintainer review; submission does not mean inclusion in a released Zigbee2MQTT version. Continue using the external converter until a release includes the built-in definition.
